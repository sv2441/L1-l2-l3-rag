...
def description_generator(df):
    ...
    summary_schema = ResponseSchema(name="Description",
                                description="Description of Action Associated in Text in 15 to 20 words.")
...
def specifications_generator(df):
    ...
    for index, row in df.iterrows():
        ...
        response = chat_llm(messages)
        data = str(response.content)  # Convert data to string
        dict_to_csv(data, 'data16.csv', append=True)
    data16 = pd.read_csv('data16.csv', names=['L1 Specifications'])
    result = pd.concat([df, data16], axis=1)
    result.to_csv('PA-results.csv', index=False)  
    st.subheader("Specifications")
    st.dataframe(result)
    final_result=pd.read_csv('results.csv',usecols=['CO Level123 Code','Level','Code','Name','Description','Prompt A','L1 Description','L1 title','L1 Intended Results',' L1 Artefact Description','L1 Specifications'])
